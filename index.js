        ...
        (err, customerResults) => {
          ...
          if (shopkeeperResults.length > 0) {
            ...
            return res.status(400).json({
              message: "Phone number already exists in shopkeepers database",
            });
          } else if (customerResults.length > 0) {
            // Phone number exists in newcustomers database
            return res.status(400).json({
              message: "Phone number already exists in newcustomers database",
            });
          } else {
...
app.post("/login", (req, res) => {
  ...
  db.query(
    "INSERT INTO sessions (user_id, session_token, login_time, expiration_time) VALUES (?, ?, ?, ?)",
...
app.get("/mainServices", (req, res) => {
  ...
  db.query(
    "SELECT * FROM tbl_salon_main_services WHERE category = ?",
    [selectedSubCategory],
    (err, results) => {
      if (err) {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (shopID) {
        ...
        db.query(
          ...
          (err, shopkeeperResults) => {
            ...
            if (shopkeeperResults.length > 0) {
              ...
              db.query(
                "INSERT INTO newcustomers (phoneNumber, name, pincode, state, city, address, shop_id, selectedCategory, selectedSubCategory, selectedCategoryType) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [
                  phoneNumber,
                  name,
                  pincode,
                ...
                (err, result) => {
                  ...
                  return res.status(200).json({
                    ...
                    shopType: shopkeeper.selectedCategory,
                  });
                }
              );
            } else {
        ...
        db.query(
          "INSERT INTO newcustomers (phoneNumber, name, pincode, state, city, address) VALUES (?, ?, ?, ?, ?, ?)",
...
app.get("/subcategories/:categoryId", (req, res) => {
  ...
  db.query(
    "SELECT * FROM nkd.tbl_salon_subcategory WHERE category_id = ?",
    [categoryId],
    (err, results) => {
      if (err) {
...
app.get("/services/subcategory/:subcategory", (req, res) => {
  ...
  db.query(
    "SELECT * FROM nkd.tbl_salon_main_services WHERE sub_category_id = ?",
    [subcategory],
    (err, results) => {
      if (err) {
...
app.get("/subservices/mainservice/:mainServiceId", (req, res) => {
  ...
  db.query(
    "SELECT * FROM nkd.tbl_salon_sub_sub_services WHERE main_service_id = ?",
    [mainServiceId],
    (err, results) => {
      if (err) {
...
app.post("/shopkeeper/selectedSubServices", (req, res) => {
  ...
  if (!phoneNumber || !mainServiceName || !subServiceName) {
    return res.status(400).json({
      message:
        "Please provide phoneNumber, mainServiceName, and subServiceName",
    });
  }
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res
        .status(200)
        .json({ message: "Selected sub-services saved successfully" });
    }
  );
});
...
app.get("/salons", (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      const customerShopIndex = results.findIndex(
        (shop) => shop.phoneNumber === shopID
      );
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length === 0) {
        return res
          .status(404)
          .json({ message: "No customer found for this phone number" });
      }
...
app.post("/shopkeeper/selectedSubServices", (req, res) => {
  ...
  if (!phoneNumber || !mainServiceName || !subServiceIds) {
    return res.status(400).json({
      message: "Please provide phoneNumber, mainServiceName, and subServiceIds",
    });
  }
    ...
    (err, results) => {
      ...
      if (results.length === 0) {
        return res.status(404).json({
          message: "Main service not found for the given sub-service ID",
        });
      }
      res.status(200).json(results[0]);
    }
...
app.get("/myServices/:phoneNumber", async (req, res) => {
  try {
    ...
    if (!queryResult || !queryResult.length) {
      console.error(
        "No selected services found for this phone number:",
        phoneNumber
      );
      return res
        .status(404)
        .json({ error: "No selected services found for this phone number." });
    }
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      if (results.length === 0) {
        ...
        return res.status(404).json({
          message: "No selected services found for this phone number.",
        });
      }
...
app.get(
  "/shopkeeper/selectedSubServices/:phoneNumber/:mainServiceId",
  (req, res) => {
    const phoneNumber = req.params.phoneNumber;
    const mainServiceId = req.params.mainServiceId;
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length === 0) {
        return res
          .status(404)
          .json({ message: "No shopkeeper found for this shop ID" });
      }
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length === 0) {
        return res
          .status(404)
          .json({ message: "No shops found for this pincode" });
      }
...
app.post("/submit-form", (req, res) => {
  ...
  const sql =
    "INSERT INTO tbl_salesexecutives (firstName, lastName, mobileNo, pincode, level) VALUES (?, ?, ?, ?, ?)";
  db.query(
    sql,
    [firstName, lastName, mobileNumber, pincode, commissionLevel],
    (err, result) => {
      if (err) {
...
app.post("/submit-team-member", (req, res) => {
  ...
  const insertSql =
    "INSERT INTO tbl_salesexecutives (mobileNo, firstName, lastName, pincode, aadhar, upi, pancard, addedBy, level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
  db.query(
    insertSql,
    [
      mobileNumber,
...
app.post("/update-profile", (req, res) => {
  const { mobileNumber, firstName, lastName, pincode, aadhar, upi, pancard } =
    req.body;
  const sql =
    "UPDATE tbl_salesexecutives SET firstName = ?, lastName = ?, pincode = ?, aadhar = ?, upi = ?, pancard = ? WHERE mobileNo = ?";
  db.query(
    sql,
    [firstName, lastName, pincode, aadhar, upi, pancard, mobileNumber],
    (err, result) => {
      if (err) {
...
app.get("/total-commission/:mobileNumber", (req, res) => {
  ...
  db.query(
    "SELECT level, addedBy FROM nkd.tbl_salesexecutives WHERE mobileNo = ?",
    [mobileNumber],
    (err, result) => {
      if (err) {
      ...
      db.query(
        "SELECT commission_amount FROM nkd.commission WHERE level = ?",
        [level],
        (err, result) => {
          if (err) {
          ...
          if (result.length === 0) {
            return res
              .status(404)
              .json({ error: "Individual commission data not found" });
          }

          const individualCommission = result[0].commission_amount;
          console.log(
            `Individual Commission for ${level}: ${individualCommission}`
          );

          // Fetch commission adjustment based on who registered the shop
          db.query(
            "SELECT commission_amount FROM nkd.commission_level WHERE from_level = ? AND to_level = ?",
            [addedBy, level],
            (err, result) => {
              if (err) {
              ...
              const adjustment = result.length
                ? result[0].commission_amount
                : 0;
              console.log(
                `Commission Adjustment from ${addedBy} to ${level}: ${adjustment}`
              );

              // Calculate total commission
              db.query(
                "SELECT COUNT(*) AS shopCount FROM shopkeepers WHERE salesAssociateNumber = ?",
                [mobileNumber],
                (err, result) => {
                  if (err) {
                  ...
                  if (level === "L1") {
                    ...
                    console.log(
                      `Total Commission for ${mobileNumber} (L1): ${totalCommission}`
                    );
                    return res.json({ totalCommission });
                  } else if (level === "L2") {
                    totalCommission =
                      (individualCommission + adjustment) * shopCount;
                    console.log(
                      `Total Commission for ${mobileNumber} (L2): ${totalCommission}`
                    );
                    return res.json({ totalCommission });
                  } else if (level === "L3") {
                    ...
                    db.query(
                      ...
                      (err, l3ToL2Result) => {
                        if (err) {
                          console.error(
                            "Error fetching commission adjustment for L3 to L2:",
                            err
                          );
                          return res
                            .status(500)
                        ...
                        const l3ToL2Adjustment = l3ToL2Result.length
                          ? l3ToL2Result[0].commission_amount
                          : 0;
                        ...
                        db.query(
                          ...
                          (err, l2ToL1Result) => {
                            if (err) {
                              console.error(
                                "Error fetching commission adjustment for L2 to L1:",
                                err
                              );
                              return res
                                .status(500)
                            ...
                            const l2ToL1Adjustment = l2ToL1Result.length
                              ? l2ToL1Result[0].commission_amount
                              : 0;
                            console.log(
                              `L2 to L1 Adjustment: ${l2ToL1Adjustment}`
                            );

                            totalCommission =
                              (individualCommission + adjustment) * shopCount;
                            const totalL2Commission =
                              l3ToL2Adjustment * shopCount;
                            const totalL1Commission =
                              l2ToL1Adjustment * shopCount;

                            console.log(
                              `Total Commission for ${mobileNumber} (L3): ${totalCommission}`
                            );
                            console.log(
                              `Total L2 Commission due to ${mobileNumber} (L3): ${totalL2Commission}`
                            );
                            console.log(
                              `Total L1 Commission due to ${mobileNumber} (L3): ${totalL1Commission}`
                            );
...
app.get("/commission/:level", (req, res) => {
  ...
  db.query(sql, [level], (err, result) => {
    ...
    if (result.length === 0) {
      res
        .status(404)
        .json({ success: false, error: "Commission data not found" });
    } else {
  ...
  try {
    const commissionRates = await new Promise((resolve, reject) => {
      db.query("SELECT * FROM commission_rates", (err, result) => {
        ...
        resolve(
          result.reduce(
            (acc, cur) => ({ ...acc, [cur.commissionType]: cur.amount }),
            {}
          )
        );
      });
...
async function updateCommissionAmount(
  mobileNumber,
  commissionType,
  commissionAmount
) {
  console.log(
    "Updating commission for:",
    mobileNumber,
    commissionType,
    commissionAmount
  );
  ...
  try {
    ...
    console.log(
      `Updated commission for ${mobileNumber} of type ${commissionType} to ${newCommissionAmount}`
    );
  } catch (error) {
...
async function assignCommission(
  ...
  try {
    ...
    if (existingCommission && existingCommission.length > 0) {
      ...
      console.log(
        `Inserted new commission for ${mobileNumber} of type ${commissionType} with amount ${commissionAmount}`
      );
    }
  } catch (error) {
...
async function checkAndAssignCommission(salesAssociateNumber) {
  console.log(
    "Checking and assigning commission for sales associate:",
    salesAssociateNumber
  );
  ...
  try {
    ...
    const addedByResult = await new Promise((resolve, reject) => {
      db.query(
        "SELECT addedBy FROM tbl_salesexecutives WHERE mobileNo = ?",
        [salesAssociateNumber],
        (err, result) => {
          if (err) {
    ...
    if (addedBy) {
      ...
      if (addedByAddedByResult && addedByAddedByResult.length > 0) {
        ...
        if (addedByAddedBy) {
          ...
          console.warn(
            `No further addedBy found for ${addedBy}, skipping L2 commission assignment.`
          );
        }
      } else {
        console.warn(
...
app.get("/products/:category", (req, res) => {
  ...
  const query =
    "SELECT id, main_category, product_name, brand_name, price, weight, picture_path FROM tbl_product_master WHERE type = ?";
...
app.post("/selectedProducts", (req, res) => {
  ...
  const query =
    "INSERT INTO tbl_my_products (phoneNumber, productId) VALUES (?, ?)";
...
app.get("/products/:category", async (req, res) => {
  try {
    ...
    const products = await db.query(
      "SELECT * FROM tbl_product_master WHERE main_category = ?",
      [category]
    );
    res.json(products);
  } catch (error) {
...
app.post("/addPreferredShop", (req, res) => {
  ...
  const sql =
    "INSERT INTO preferred_shops (customerPhoneNumber, shopID, shopkeeperName, phoneNumber, selectedCategory, shopType, pincode) VALUES (?, ?, ?, ?, ?, ?, ?)";
  db.query(
    sql,
    [
      customerPhoneNumber,
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Failed to add preferred shop" });
      }
...
app.delete("/removePreferredShop", (req, res) => {
  ...
  const sql =
    "DELETE FROM preferred_shops WHERE customerPhoneNumber = ? AND shopID = ?";
  db.query(sql, [customerPhoneNumber, shopID], (err, result) => {
    if (err) {
      console.error("Error removing preferred shop:", err);
      return res
        .status(500)
        .json({ message: "Failed to remove preferred shop" });
    }
...
app.get("/customer/address", (req, res) => {
  ...
  db.query(query, [phoneNumber], (error, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: "Address not found for this phone number" });
    }
